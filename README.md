...
[Запустить](#быстрый-запуск) · [Проверить за 5 минут](#как-проверить-решение) · [Что делает AI](#как-работает-решение) · [Архитектура](#архитектура) · [Методика](docs/MODEL.md)

![QALA: предпросмотр города и диалог с AI рядом; план применяется только по кнопке](docs/screenshots/planner.png)

_Результат подбора сразу виден на карте. Принятый план изменится только после нажатия «Применить план»._
...
<details>
<summary><strong>Ещё два экрана: обзор города и эксперимент задержки</strong></summary>
...
flowchart LR
  ...
  AI --> C["Проверка полноты перевода запроса"]
  C --> API
