...
describe("published dataset", () => {
  ...
  it("reproduces the independently verified baseline", () => {
    expect(BASELINE.districts.map((d) => +d.score.toFixed(2))).toEqual([
...
describe("rules", () => {
  ...
  it("rejects all three conflicts and permits local measures in different districts", () => {
    for (const pair of [
      ...
      expect(
        validate(
          pair.map((measureId) => ({ measureId, districtId: "nura" })),
          false,
        ).valid,
      ).toBe(false);
    }
  ...
  it("requires valid districts and forbids a district for city measures", () => {
    for (const item of [
      { measureId: "M7" },
      { measureId: "M7", districtId: "fake" },
      { measureId: "M12", districtId: "nura" },
    ])
      expect(validate([item], false).valid).toBe(false);
  });
  it("rejects a third measure in the same direction", () => {
    expect(
      validate(
        ["M7", "M8", "M9"].map((measureId) => ({ measureId, districtId: "nura" })),
        false,
      ).valid,
    ).toBe(false);
  });
  it("handles untrusted malformed input without crashing", () => {
    for (const input of [
      ...
      expect(evaluate(input).result).toBeNull();
  });
});
...
describe("effects and fairness", () => {
  ...
  it("charges city interventions once and applies them everywhere", () => {
    ...
    result.districts.forEach((d, i) =>
      expect(d.indicators.C2 - DISTRICTS[i].indicators.C2).toBe(4.375),
    );
  });
  it.each([
    ["M1", "M2", "T1", 9.5],
    ["M5", "M6", "E2", 12.25],
    ["M10", "M12", "B1", 12.5],
  ] as const)("applies %s + %s synergy without lag scaling", (a, b, k, delta) => {
  ...
  it("includes negative tradeoffs", () => {
    expect(simulate([{ measureId: "M11", districtId: "nura" }]).districts[4].indicators.T1).toBe(
      53.25,
    );
  });
...
describe("explainability and recommendations", () => {
  it("allocates the whole score improvement using exact Shapley values", () => {
    ...
    expect(values.reduce((s, v) => s + v.contribution, 0)).toBeCloseTo(
      simulate(EXAMPLE).score - BASELINE.score,
      10,
    );
    const reversed = attribute([...EXAMPLE].reverse());
    values.forEach((v) =>
      expect(reversed.find((r) => r.measureId === v.measureId)?.contribution).toBeCloseTo(
        v.contribution,
        10,
      ),
    );
  ...
  it("returns only a valid, affordable, strictly better single replacement", () => {
    ...
    expect(
      alt!.decisions.filter((d, i) => JSON.stringify(d) !== JSON.stringify(EXAMPLE[i])),
    ).toHaveLength(1);
  ...
  it("rejects malformed shared scenarios and round-trips valid selections", () => {
    ...
    expect(simulate(parseScenario(scenarioKey(EXAMPLE))!).score).toBeCloseTo(
      simulate(EXAMPLE).score,
      12,
    );
    for (const s of ["oops", "M7:fake", "M12:nura", "M7:nura:extra", "__proto__:city"])
      expect(parseScenario(s)).toBeNull();
  });
});
